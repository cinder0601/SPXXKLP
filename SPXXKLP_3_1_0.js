...
// @include     https://mobile.x.com/*/status/*
...
(function () {
  ...
  function getHeader(articleType, type) {
    if (articleType.toLowerCase() !== 'news') {
      return `[align=left][color=#388e3c][size=5]|[/size][/color][size=4]本文内容按照 [/size][url=https://creativecommons.org/licenses/by-nc-sa/4.0/deed.zh-hans][size=4][color=#2e8b57]CC BY-NC-SA 4.0[/color][/size][/url][/font][size=4] 协议进行授权，[b]转载本帖时须注明[color=#ff0000]原作者[/color]以及[color=#ff0000]本帖地址[/color][/b]。[/font][/size][/align][/align][/align][/align][hr]\n`;
    }
...
[color=#388e3c][size=5]|[/size][/font][/color][size=4]本文内容按照 [/font][/size][url=https://creativecommons.org/licenses/by-nc-sa/4.0/deed.zh-hans][size=4][color=#2e8b57]CC BY-NC-SA 4.0[/color][/size][/url][/font][size=4] 协议进行授权，[b]转载本帖时须注明[color=#ff0000]原作者[/color]以及[color=#ff0000]本帖地址[/color][/b]。[/font][/size][hr]\n`;
...
/*Same contents,change if necessary.**/
    ...
    switch (type) {
      case VersionType.Snapshot:
        return `\n${poweredBy}\n[hr][color=#388e3c][size=5]|[/size][/color][size=4][b]想了解更多游戏资讯？[/b][/size][list][*][size=3][url=https://klpbbs.com/forum-2-1.html][color=#388e3c]苦力怕论坛 - 游戏资讯版块[/color][/url][/size][/list][/indent]`;
...
【本文排版借助了：[url=https://github.com/cinder0601/SPXXKLP][color=#388d40][u]SPXXKLP[/u][/color][/url] Userscript v${spxxklpVersion}】[/indent][/indent]\n\n${footer}`;
  ...
  function getTweetMetadata() {
      ...
      const posterNameElement = document.querySelector('div[data-testid="User-Name"] a span');
      if (posterNameElement) {
          tweetMetadata.userName = posterNameElement.textContent;
      } else {
          tweetMetadata.userName = null;
      }
      let texts = [];
      const articleDivs = document.querySelector('article div[lang]').querySelectorAll('span');
      for (const span of articleDivs) {
          let textContent = span.textContent;
          const links = span.querySelectorAll('a');
          links.forEach(link => {
              const url = link.href;
              const linkText = link.textContent;
              const linkRegex = new RegExp(linkText, 'g');
              textContent = textContent.replace(linkRegex, `[url=${url}]${linkText}[/url]`);
          });
  ...
  function getTweetBbcode(tweet, mode) {
      ...
      const dateString = `${tweet.date} · ${tweet.source} · SPXXKLP v${spxxklpVersion}`;
  ...
  [size=15px]由 <译者名> 翻译自${tweet.lang.startsWith('en') ? '英语' : ` ${tweet.lang}`}[/size]
  [size=23px]<请填写译文>
  [/size][/color][/indent][align=center]<如有配图，请在此处添加>[/align]
  [indent][size=15px][url=${tweet.tweetLink}][color=${attributeColor}]${dateString}[/color][/url][/size][/indent][/font][/td][/tr]
  [/table][/align]`;
  }
