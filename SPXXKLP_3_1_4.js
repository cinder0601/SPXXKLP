...
(function () {
  ...
  function getVersionType(url) {
    ...
    } else if (lowerUrl.includes('minecraft-java-edition') && !lowerUrl.includes('snapshot')) {
      return VersionType.Release;
    } else if (lowerUrl.includes('minecraft-preview') || lowerUrl.includes('minecraft-beta-preview') || lowerUrl.includes('minecraft-beta')) {
  ...
  function getVersionCode(url){
    ...
    if (lowerUrl.includes('snapshot')) {
      ...
      const versionRegex = /(\d+)\-(\d+)\-pre-release\-(\d+)/;
      const match = lowerUrl.match(versionRegex);
      if (match && match.length >= 4) {
        const majorVersion = match[1];
        const minorVersion = match[2];
        const preReleaseNumber = match[3];
        const formattedVersion = `${majorVersion}.${minorVersion}-pre${preReleaseNumber}`;
        return formattedVersion;
      }
    } else if (lowerUrl.includes('release-candidate')) {
      const versionRegex = /(\d+)\-(\d+)\-release-candidate\-(\d+)/;
      const match = lowerUrl.match(versionRegex);
      if (match && match.length >= 4) {
        const majorVersion = match[1];
        const minorVersion = match[2];
        const preReleaseNumber = match[3];
        const formattedVersion = `${majorVersion}.${minorVersion}-rc${preReleaseNumber}`;
        return formattedVersion;
      }
...
[color=#f44336][size=5]|[/size][/color][size=4]<正式版版本号> 仍未发布，${versioncode} 为其第 <计数> 个预览版。[/size]
[color=#388e3c][size=5]|[/size][/font][/color][size=4]本文内容按照 [/font][/size][url=https://creativecommons.org/licenses/by-nc-sa/4.0/deed.zh-hans][size=4][color=#2e8b57][u]CC BY-NC-SA 4.0[/u][/color][/size][/url][/font][size=4] 协议进行授权，[b]转载本帖时须注明[color=#ff0000]原作者[/color]以及[color=#ff0000]本帖地址[/color][/b]。[/font][/size][hr]\n`;
...
/*Same contents,change if necessary.**/
  ...
  async function minecraftNet() {
    ...
    if (url.match(/^https:\/\/www\.minecraft\.net\/(?:[a-z-]+)\/article\//||/^https:\/\/help\.minecraft\.net\/(?:[a-z-]+)\/(?:[a-z-]+)\/article\//)) {
  ...
  function getHelpContent(controlDOM) {
    ...
    button.onclick = async () => {
        ...
        let title = bbcode;
        title = title.replace(/\n/g, '');
        bbcode = `[align=left][color=#388e3c][size=5]|[/size][/color][size=4]本文内容按照 [/size][url=https://creativecommons.org/licenses/by-nc-sa/4.0/deed.zh-hans][size=4][color=#2e8b57][u]CC BY-NC-SA 4.0[/u][/color][/size][/url][/font][size=4] 协议进行授权，[b]转载本帖时须注明[color=#ff0000]原作者[/color]以及[color=#ff0000]本帖地址[/color][/b]。[/font][/size][/align][/align][/align][/align][hr]\n[size=6][b][color=silver]${bbcode}[/color][/b][/size][size=6][b]${bbcode}[/b][/size]\n`
        bbcode += await converthelpElementsToBBCode(content, ctx);
        bbcode += `[b]【${ctx.translator} 译自[url=${ctx.url}][color=#388d40][u] help.minecraft.net 上的 ${title}[/u][/color][/url]】\n【本文排版借助了：[url=https://github.com/cinder0601/SPXXKLP][color=#388d40][u]SPXXKLP[/u][/color][/url] 用户脚本 v${version}】[/b]`;
